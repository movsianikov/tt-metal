...
RingTopology::RingTopology(
    ...
    for (uint32_t l = 0; l < num_links; ++l) {
        ...
        if (!is_linear || ring_index != ring_size - 1) {
            ...
            const auto& sockets = device->get_ethernet_sockets(receiver_device, /*skip_reserved_fabric_cores=*/true);
        ...
        if (!is_linear || ring_index != 0) {
            ...
            const auto& sockets = device->get_ethernet_sockets(sender_device, /*skip_reserved_fabric_cores=*/true);
