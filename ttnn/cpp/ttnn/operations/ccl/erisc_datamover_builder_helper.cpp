...
EdmLineFabricOpInterface::EdmLineFabricOpInterface(
    ...
    auto get_min_link_count = [&](IDevice* src_device, IDevice* dest_device, size_t min_link_count) {
        const auto& src_device_sockets =
            src_device->get_ethernet_sockets(dest_device->id(), /*skip_reserved_fabric_cores=*/true);
        const auto& dest_device_sockets =
            dest_device->get_ethernet_sockets(src_device->id(), /*skip_reserved_fabric_cores=*/true);
...
EdmLineFabricOpInterface::EdmLineFabricOpInterface(
    ...
    for (size_t i = 0; i < device_pairs.size(); i++) {
        ...
        const auto connected_sockets =
            local_device->get_ethernet_sockets(remote_device->id(), /*skip_reserved_fabric_cores=*/true);
        // re-order the connected_sockets based on virtual coords
